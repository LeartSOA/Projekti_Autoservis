...
public class NewServiceActivity extends NewBaseActivity {
    ...
    public boolean isInputValid() {
        ...
        if (type.length() > 36) {
            valid = false;
            tilType.setError("Tipi shume i gjate");
        }else {
            if (!StringUtils.isAlphanumericSpace(type)) {
                valid = false;
                tilType.setError("Lejohen vetem shkronja");
            } else if (type.isEmpty()) {
                valid = false;
                tilType.setError("Kjo fushe nuk mund te jete e zbrazet");
            }
        }
        ...
        if (toggleButton.isChecked()) {
            if (DateUtils.isNotValidDate(btnNotificationDate.getText().toString(), false)) {
                valid = false;
                showMessage("Date jo e sakte e njoftimit");
            }else if (!DateUtils.isExpirationDateValid(btnNotificationDate.getText().toString())) {
                valid = false;
                showMessage("Data e njoftimit duhet te jete te pakten nje dite me vone se data aktuale");
            }
        }else {
            if (!NumberUtils.isCreatable(TextUtils.getTextFromTil(tilOdometerNotification))) {
                valid = false;
                tilOdometerNotification.setError("Lejohet vlere numerike");
            }else {
                if (NumberUtils.createLong(TextUtils.getTextFromTil(tilOdometerNotification)) < getVehicleOdometer()) {
                    valid = false;
                    tilOdometerNotification.setError("Kilometrazhi pritet te jete me i madh se ai aktual");
                }
            }
        }
    ...
    protected void onItemSaved() {
        if (isNewItem()) {
            showMessage("Riparimi u ruajt");
        }else {
            showMessage("Riparimi u rifreskua");
    ...
    private void setNotification(Service service) {
        ...
        Notification notification = NotificationUtils.createNotification
                (
                        ...
                        "Service", service.getType().getName() + " duhet te shikohet me " + DateUtils.datetimeToString(service.getDateNotification().getDate()),
